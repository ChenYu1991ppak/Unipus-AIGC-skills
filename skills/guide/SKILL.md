...
> 这条名字在本仓库早期叫"译后编辑"，是**错的**。

**用 Skill 工具去唤起**，不要自己照着文档拼命令——应用 skill 里有各自的
坑位清单和参数引导。

平台上总共 25 个应用，**还有十几个尚未实现**。用户问到它们时，**照内部维护的
应用清单回答**（那份按可做性分了档：哪些"接口文档已经给全、照着骨架就能补"、
哪些"要重新抓包"、哪些"不建议做"）。那份清单**不随仓库发布**。
**不要对用户谎称某个未实现的应用能用。**
...
完整的接口级排查材料在**内部维护的接口记录**里（逐条调用链 + 字段名踩坑表），
以及**内部的应用清单**（25 个应用全景）。这两份是内部资料，**不随仓库发布**。
